...
{
	/*
	 * SR-IOV VFs will use the group of the PF if it has
	...
	/*
	 * PCIe r7.0, sec 6.12.1.2 ACS Functions in SR-IOV, SIOV, and
...
static struct iommu_group *pci_get_function_group(struct pci_dev *pdev)
{
	 ...
	 * group.
	 *
	 * pci_reachable_bus_set() should return the same bitmap if called for
	 * any device in the set and we want all devices in the set to have the
	 * same group.
	 */
	pci_reachable_bus_set(pdev, &devfns, pci_devs_are_same_group);
...
static struct iommu_group *pci_hierarchy_group(struct pci_dev *pdev)
{
	...
	/*
	 * !self is only for SRIOV virtual buses which should have been
	...
	if (!group) {
		 ...
		 * there is no way for its pci_device_group() to discover it.
		 */
		dev_err(&pdev->dev,
 ...
 *
 *  1) In PCI we can lose the RID inside the fabric, or some devices will use
 ...
 *     each other. In PCIe the ACS system can be used to inhibit this and
