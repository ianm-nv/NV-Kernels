							 ...
							 bool coherent)
{
	...
	if ((resource_id >= PCI_NUM_RESOURCES) ||
	    (start < pci_resource_start(dev, resource_id)) ||
	    (start > pci_resource_end(dev, resource_id)) ||
	    ((start + size - 1) > pci_resource_end(dev, resource_id)))
		return NULL;
	...
	start_phys += start - pci_resource_start(dev, resource_id);
