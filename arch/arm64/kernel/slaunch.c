 ...
 * slaunch_validate_raw_systab() and consumed by slaunch_measure_srtm_log()
 * to bind the log into the DRTM event log. Zero (no-op) when firmware did
 * not publish the GUID (e.g. FVP without Tcg2Dxe).
 */
 ...
 * overlap with DLME/DTB/mmap so firmware cannot alias an input. On
 * success captures sl_srtm_log_pa/size for slaunch_measure_srtm_log().
 */
					     ...
					     const efi_guid_t *guid)
{
	...
	if (is_main) {
		 ...
		 * algorithm-set descriptor we may not have seen, so cap at
		 * SL_SRTM_LOG_MAX and treat events[] as opaque. version must
		 * be 1 per TCG2 ACPI spec; reject otherwise before nr_events.
		 */
	...
	if (body_size > SL_SRTM_LOG_MAX - hdr_size)
		panic("slaunch: SRTM log size %llu exceeds 1 MiB cap (header %zu B + body)\n",
		      body_size, hdr_size);
...
static void __init slaunch_validate_raw_systab(u64 systab_pa)
{
	...
	for (j = 0; j < nr_tables; j++) {
		 ...
		 * cover the full body extent and remember the validated
		 * (PA, size) for slaunch_measure_srtm_log(). */
...
static void __init slaunch_verify_hash_algo(void)
{
	...
	/* TODO: extend to SHA-384/SHA-512 via crypto_shash; currently SHA-256 only. */
...
/*
 * srtm_log_huge_size needs a real eventlog header at a known-NORMAL PA
 * not overlapping the DLME region so the validator reaches the size cap.
 * Use a scratch PA in FVP's high DRAM bank, written via early_memremap
 * before the validator runs.
 */
#define SL_INJECT_SRTM_SCRATCH_PA	0x890000000ULL

static bool __init slaunch_inject_srtm_write_hdr(u64 pa, u32 size_field)
{
	struct linux_efi_tpm_eventlog *hdr;

	hdr = early_memremap(pa, sizeof(*hdr));
	if (!hdr) {
		pr_warn("slaunch: INJECT srtm: scratch remap failed at 0x%llx\n",
			pa);
		return false;
	}
...
static void __init slaunch_inject_srtm_log(const struct sl_efi_info *info)
{
	...
	if (sl_cmdline_has("slaunch_inject=srtm_log_huge_size")) {
		if (!slaunch_inject_srtm_write_hdr(SL_INJECT_SRTM_SCRATCH_PA,
						   SL_SRTM_LOG_MAX + 1))
			return;
		vendor_pa = SL_INJECT_SRTM_SCRATCH_PA;
		tag = "srtm_log_huge_size";
	} else if (sl_cmdline_has("slaunch_inject=srtm_log_overlap_dlme")) {
...
/*
 * Measure the validated UEFI SRTM TPM event log into the DLME-side table;
 * slaunch_extend_drtm_event_log() folds it onto PCR 18. A verifier
 * replays the same hash from a reference to detect tampering. No-op when
 * firmware published no SRTM-log GUID (FVP today, no Tcg2Dxe).
 */
static void __init slaunch_measure_srtm_log(void)
{
	void *log;

	if (!sl_srtm_log_pa) {
		pr_info("slaunch: no UEFI SRTM TPM event log published — skipping measurement\n");
		return;
	}

	log = early_memremap_ro(sl_srtm_log_pa, sl_srtm_log_size);
	if (!log)
		panic("slaunch: failed to map SRTM TPM event log at 0x%llx for measurement\n",
		      (u64)sl_srtm_log_pa);
	slaunch_measure("SRTM_TPM_LOG", log, sl_srtm_log_size);
	early_memunmap(log, sl_srtm_log_size);
}
