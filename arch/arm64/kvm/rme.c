...
static int rme_mecid_init(void)
{
	return __mecid_init(&mecid_state);
}

static void rme_mecid_destroy(void)
{
	__mecid_destroy(&mecid_state);
}

static int rme_mecid_reserve(struct realm *realm)
{
	int ret = 0;
...
static void rme_mecid_release(unsigned int mecid)
{
	if (!mecid_supported())
		return;
...
static int kvm_rme_config_realm(struct kvm *kvm, struct kvm_enable_cap *cap)
{
	...
	if (cfg.cfg == ARM_RME_CONFIG_MEC_QUERY) {
