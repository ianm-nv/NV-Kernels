...
struct pci_dev {
	...
	struct pci_ide *ide;		/* Integriry and Data Encryption */
#endif
...
/*
 * set of devices on a single bus. Each bit represents a device in the set using
 * pdev->devfn as the bitmap index.
 */
...
static inline void pci_reachable_bus_set(
	struct pci_dev *start, struct pci_reachable_set *devfns,
